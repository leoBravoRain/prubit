			...
			if(commentString){
				...
				$.ajax({
					...
					success: function(response){
						...
						// {id de usuario: lista de 1 objeto foto de perfil}
						var profilePhotosOfUserOfCommentsToGarmentCompanyPosts = response.profilePhotosUsersOfComments;

						// Se utiliza el servicio commentOfPostsService para adicionar la informacion necesaria para el comentario
						commentOfPostsService.addInformationToComment(comment,usersOfComment,myId,profilePhotosOfUserOfCommentsToGarmentCompanyPosts,urlMedia,{});
