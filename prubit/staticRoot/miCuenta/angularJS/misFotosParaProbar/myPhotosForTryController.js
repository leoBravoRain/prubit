posts.controller("postsController",function($scope){
	...
	// funcion que se ejecuta cada vez que se alncaza el final de la ventana
	...
	function addDataToView(response){
		...
		// 		var postsNew =photos[myId];

		// 		for (var i=0;i<photosUser.length;i++){

		// 			// Se crea el postObject para agregar la foto
		// 			var postObject = {"post":photosUser[i]};

		// 			// Se agrega la foto (y todo lo relacionado a esta) a la lista postsList la cual se muestra en el temlpate
		// 			$scope.$apply(function(){
